...
impl QemuParams {
    pub fn new_from_snapshot(workdir: &str, qemu_id: usize, cpu: usize, params: &SnapshotVmParams, create_snapshot_file: bool) -> QemuParams{
        ...
        cmd.push("can-bus,id=canbus0".to_string());
        cmd.push("-object".to_string());
        cmd.push("can-host-socketcan,id=canhost0,if=can0,canbus=canbus0".to_string());
        cmd.push("-device".to_string());
        cmd.push("kvaser_pci,canbus=canbus0".to_string());
