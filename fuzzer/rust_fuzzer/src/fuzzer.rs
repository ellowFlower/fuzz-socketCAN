...
impl FuzzStats{
    ...
    pub fn write_crash_time(&mut self, cfg: &FuzzerConfig){
        let mut file = OpenOptions::new()
        .append(true)
        .create(true)
        .open(format!("{}/crash_times.txt", cfg.workdir_path))
        .unwrap();
...
impl<Fuzz: FuzzRunner + GetStructStorage> StructFuzzer<Fuzz> {
    ...
    fn new_input(&mut self, input: &Input) {
        ...
        if input.exit_reason.name() == "crash" {
            self.stats.write_crash_time(&self.config);
        }
