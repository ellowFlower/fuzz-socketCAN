...
//#endif
    ...
    */
    ...
    for (int i = 0; i < num_copied; i++) {
        hprintf("%02X ", data_buffer[next_data]);
    }
    hprintf("\n num_copied: %d \n", num_copied);
    */
...
ssize_t call_vm_can(void *data, size_t max_size, bool return_pkt_size, bool disable_dump_mode, int client_socket){
...
//#endif
    ...
    if(available_data == 0){
        ...
        // check if can or can fd
    int enable_fd;
    socklen_t optlen = sizeof(enable_fd);
    if (getsockopt(client_socket, SOL_CAN_RAW, CAN_RAW_FD_FRAMES, &enable_fd, &optlen) < 0) {
        perror("Socket option reading error");
        return -1;
    }

    struct can_filter used_can_filter;
    optlen = sizeof(used_can_filter);
    if (getsockopt(client_socket, SOL_CAN_RAW, CAN_RAW_FILTER, &used_can_filter, &optlen) == -1) {
        DEBUG("%s: cannot read socket options\n", __func__);
        return -1;
    }
    ...
    if (enable_fd == 1) {
        ...
        max_size = 64;
        return_value = sizeof(struct canfd_frame);
        num_copied = min_size_t(available_data, max_size);
        frame.can_id = used_can_filter.can_id;
        frame.len = num_copied;
        memcpy(frame.data, &data_buffer[next_data], num_copied);
        memcpy(data, &frame, return_value);
    } else {
        // can
        struct can_frame frame;
        max_size = 8;
        return_value = sizeof(struct can_frame);
        num_copied = min_size_t(available_data, max_size);
        frame.can_id = used_can_filter.can_id;
        frame.len = num_copied;
        memcpy(frame.data, &data_buffer[next_data], num_copied);
...
//#ifdef COPY_PAYLOAD_MODE  
    if(payload_mode && !disable_dump_mode){
        hprintf_payload(data, return_value);
    }
//#endif
    //return 0;
    return return_value;
