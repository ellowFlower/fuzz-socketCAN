...
extern ssize_t call_vm_can(void *data, size_t max_size, bool return_pkt_size, bool disable_dump_mode, int client_socket);
...
int handle_next_packet_can(int client_socket, void* buf, size_t len, bool return_pkt_size){
	if(client_socket != -1){
		DEBUG("%s: fd: %d buf: %p size: %d, rps: %d\n", __func__, client_socket, buf, len, return_pkt_size);
		ssize_t data_len = call_vm_can(buf, len, return_pkt_size, false, client_socket);
...
ssize_t read(int fd, void *buf, size_t count){
	...
	if (connection_is_can(fd)) {
		init_nyx();
		return handle_next_packet_can(fd, buf, 8, false);
	} else {
		init_nyx();
...
ssize_t recv(int sockfd, void *buf, size_t len, int flags){
	...
	if(server_socket_exists(sockfd)){
#ifndef NET_STANDALONE
	if (connection_is_can(sockfd)) {
		init_nyx();
		return handle_next_packet_can(sockfd, buf, 8, false);
	} else {
		init_nyx();
...
ssize_t recvfrom(int sockfd, void *buf, size_t len, int flags, struct sockaddr *src_addr, socklen_t *addrlen){
	...
	if (connection_is_can(sockfd)) {
		init_nyx();
		return handle_next_packet_can(sockfd, buf, 8, flags & MSG_TRUNC);
	} else {
		init_nyx();
		return handle_next_packet(sockfd, buf, len, flags & MSG_TRUNC);
	}
#endif
	}
...
int listen(int sockfd, int backlog){
	...
	// getsockname: returns current address of sockfd and saves it in addr
	if(getsockname(sockfd, (struct sockaddr *) &addr, (void*)&len) != -1){

		// check if connections variable contains port (nyx-net specific)
		...
		if(!exists){
				// check if port provided by --nyx-net-port option when packing target
				if(is_target_port(ntohs(addr.sin_port))){ 
					// add port to variable connections
					add_connection(ntohs(addr.sin_port));
					// nyx-net connects as client to server
...
int bind(int sockfd, const struct sockaddr *addr, socklen_t addrlen) {
	...
	if (connection_is_can(sockfd)){
		if (!connection_exists_can()) {
			add_connection_can();
		}
		
		if (ret != -1) {
			set_server_socket_to_connection_can(sockfd);	
		}
	}
