...
// connection for a specific port or can interface
typedef struct interfaces_s {
	...
	uint16_t port_server; // not used
	uint16_t port_client; // not used
	uint16_t port; // port server listens to
	bool is_can;
  	bool disabled;
...
bool set_server_socket_to_connection_can(int socket){
	//DEBUG("+++++++++++++++++++++++ socketcache %s: %d\n", __func__, socket);
...
bool add_connection_can(){
	//DEBUG("+++++++++++++++++++++++ socketcache %s\n", __func__);
	
	assert(!connection_exists_can());
	...
	connections[active_connections].is_can = true;
	connections[active_connections].client_sockets_num = 0;
	connections[active_connections].server_sockets_num = 0;
	active_connections++;
  	active_con_num++;
